...
from model_resnet import ResNet, resnet18, resnet34, resnet50
...
if __name__ == '__main__':
    with open("F:\pac-2019-master (2)\pac-2019-master\config.json") as fid:
        ctx = json.load(fid)

    if ctx["3d"]:
        train_set = PAC20193D(ctx, set='train')
        val_set = PAC20193D(ctx, set='val')
        model = ColeModel().to(device)
    ...
    for e in tqdm(range(1, ctx["epochs"]+1), desc="Epochs"):
        ...
        for i, data in enumerate(train_loader):
            ...
            input_image = Variable(data["input"], requires_grad=True).float().to(device)
        ...
        for i, data in enumerate(val_loader):
            input_image = Variable(data["input"]).float().to(device)
            output = model(input_image)
            label = Variable(data["label"].float()).to(device)
